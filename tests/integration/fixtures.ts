...
export function simulateEffects(
	...
	const before = clonePlayer(context.activePlayer);
	const dummy = clonePlayer(context.activePlayer);
	const dummyContext = { ...context, activePlayer: dummy } as EngineContext;
	runEffects(effects, dummyContext);
	if (actionId) {
		context.passives.runResultModifiers(actionId, dummyContext);
	}
...
export function getActionOutcome(id: string, context: EngineContext) {
	const actionDefinition = context.actions.get(id);
	const costs = getActionCosts(id, context);
