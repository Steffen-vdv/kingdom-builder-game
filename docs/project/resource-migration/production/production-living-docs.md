...
| Resource Migration MVP - P3 - T19 | 2025-10-23 | gpt-5-codex | packages/web/src/contexts/registryMetadataDescriptors.ts; packages/web/src/contexts/registryMetadataBuilders.ts | Restored the registry metadata descriptor barrel by re-exporting the builder utilities/types and introduced helper aliases to satisfy the web lint max-len requirements for ResourceV2 metadata merging. | `npm run lint --workspace=@kingdom-builder/web`; `npm run test --workspace=@kingdom-builder/web -- --reporter=dot` _(fails: Vitest cannot resolve @kingdom-builder/testing and several React act-driven suites abort; investigate module resolution and test harness setup before rerun)_ | Re-run the web test suite once the Vitest resolver recognises the local testing package and the React act helpers are configured for the new ResourceV2 selectors. |
...
**Timestamp (UTC):** 2025-10-23 19:40
**Current Focus:** Resource Migration MVP - P3 - T19 Web translation metadata cleanup
**State Summary:** Restored the registry metadata descriptor surface after the builder/type extraction, added shared aliases to rein in lint max-len noise, and confirmed the new ResourceV2 utilities compile inside the translation layer. Workspace linting now passes, but Vitest is still tripping over unresolved local packages and missing React test harness configuration.

- **Next Suggested Tasks:**
  - Wire the updated `registryMetadataDescriptors` barrel into the remaining callers and prune any redundant imports left from the refactor.
  - Fix Vitest resolution for `@kingdom-builder/testing`/ResourceV2 helpers (likely by ensuring the workspace builds run or adding explicit path aliases) so the web suite exercises the new selectors.
  - Stabilise the React testing environment (`act(...)` warnings) before re-running the full web suite to validate the translation updates end-to-end.
- **Blocking Issues / Risks:** Web tests currently fail because Vitest cannot locate `@kingdom-builder/testing` and several React suites emit `act(...)` errors; unblock this before shipping to prevent masking regressions in the new ResourceV2 translators.
