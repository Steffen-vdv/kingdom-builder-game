...
- **Current Focus:** Resource Migration MVP - P2 - T8 - Implement ResourceV2 reconciliation helpers
- **State Summary:** Added pure helpers that compute static or percentage deltas with explicit rounding, perform clamp reconciliation against configured bounds, and expose shared change/reconciliation types through the engine ResourceV2 entry point. Documented follow-up wiring needs in the living doc and confirmed the toolchain with `npm run format`, `npm run lint`, `npm run check`.
- **Next Suggested Tasks:**
  - Integrate the reconciliation helpers into upcoming `resource:add/remove/transfer` effect handlers and ensure logging captures clamp signals (Owner: Engine).
  - Backfill targeted unit tests covering rounding edge cases (negative values, tie behaviour) once handlers are wired (Owner: Engine/Test).
- **Blocking Issues / Risks:** Downstream handlers still rely on legacy resource math; until they adopt the helpers there is no single source of truth for rounding and reconciliation behaviour.
