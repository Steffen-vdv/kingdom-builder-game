...
| Resource Migration MVP - P3 - T14 | 2025-10-24 | gpt-5-codex | packages/engine/src/actions/costs.ts; packages/engine/src/resourceV2/service.ts; packages/engine/tests/actions/costs.global-action-cost.test.ts | Enforced ResourceV2 global action cost metadata by rejecting overrides/omissions, applying defaulted amounts, and adding coverage for modifier interactions. | `npm run lint`; `npm run test --workspace=@kingdom-builder/engine` | Wire the engine bootstrap to load live ResourceV2 registries so metadata-driven costs apply without manual test hooks. |
...
**Timestamp (UTC):** 2025-10-24 14:25
**Current Focus:** Resource Migration MVP - P3 - T14 Global action cost enforcement
**State Summary:** Engine cost calculations now pull ResourceV2 global-action-cost metadata, force non-system actions to match the configured amount, prevent collectors from stripping the key, and ensure passives still modify the enforced baseline. Added targeted tests for success, override/omission failures, and modifier interplay.

- **Next Suggested Tasks:**
  - Load the ResourceV2 registry during engine bootstrap so production sessions automatically enforce global cost metadata.
  - Coordinate with content to remove redundant AP entries once registry-driven enforcement is live.
- **Blocking Issues / Risks:** Until the registry feeds into engine startup, metadata enforcement only activates in contexts that manually seed the ResourceV2 registry.
