...
| Resource Migration MVP - P3 - T14 | 2025-10-24 | gpt-5-codex | `packages/engine/src/actions/costs.ts`; `packages/engine/src/resourceV2/registry.ts`; `packages/engine/src/resourceV2/service.ts`; `packages/engine/src/setup/create_engine.ts`; `packages/engine/src/setup/player_setup.ts`; `packages/engine/tests/actions/global-action-costs.test.ts`; `docs/project/resource-migration/production/production-living-docs.md` | Enforced ResourceV2 global action cost metadata throughout the engine pipeline so non-system actions inherit the configured cost, validated registry wiring, and added regression coverage for cost modifiers and override rejection. | `npm run lint`; `npm run test --workspace=@kingdom-builder/engine` (pass) | Monitor legacy configurations lacking ResourceV2 metadata to ensure fallback behaviour stays stable and coordinate with content to flag the global cost resource. |
...
**Timestamp (UTC):** 2025-10-24 15:42
**Current Focus:** Resource Migration MVP - P3 - T14 Global action cost enforcement
**State Summary:** Engine action cost resolution now consumes ResourceV2 metadata for the designated global cost resource, auto-populating non-system action costs, preventing base-cost overrides, and syncing the registry through engine creation. Tests cover enforcement, modifier adjustments, and legacy fallbacks.

- **Next Suggested Tasks:**
  - Validate content registries define the global action cost resource and migrate action definitions away from manual AP base costs.
  - Extend web/UI snapshots to highlight the enforced cost resource so designers can confirm live metadata.
- **Blocking Issues / Risks:** Legacy setups without ResourceV2 metadata still rely on historic defaults; coordinate rollout to avoid mixed configurations during migration.
