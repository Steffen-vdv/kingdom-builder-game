...
| Resource Migration MVP - P3 - T20 | 2025-10-23 | gpt-5-codex | packages/web/src/components/resources/ResourceHud.tsx; packages/web/src/components/player/PlayerPanel.tsx; packages/web/tests/components/resources/ResourceHud.test.tsx; docs/project/resource-migration/production/production-living-docs.md | Implemented the Resource V2 HUD with group/virtual-parent rollups, touched-state visibility, percent formatting, and bound/tier badges, replaced the player panel HUD wiring while preserving the legacy fallback, and documented the migration checkpoint. | `npm run lint --workspace=@kingdom-builder/web`; `npm run test --workspace=@kingdom-builder/web` _(fails: Vitest cannot resolve @kingdom-builder/engine and @kingdom-builder/testing package entries in the sandbox workspace)_ | Publish/alias engine & testing packages so Vitest resolves them, re-run the full web suite, and expand HUD coverage for parent metadata fallbacks once fixtures exist. |
...
**Timestamp (UTC):** 2025-10-23 20:35
**Current Focus:** Resource Migration MVP - P3 - T20 Resource HUD integration
**State Summary:** The web client now renders Resource V2 values through the new HUD with group rollups, touched-state filtering, percent-aware amounts, and tier/bound badges, and the player panel swaps between the V2 HUD and legacy bar to keep migration-safe fallbacks while tests cover grouping, totals, and formatting behaviour.

- **Next Suggested Tasks:**
  - Restore Vitest’s ability to resolve `@kingdom-builder/engine` and `@kingdom-builder/testing` so `npm run test --workspace=@kingdom-builder/web` completes, then capture the full run output.
  - Backfill HUD coverage for parent metadata fallbacks and multi-tier tracks once representative fixtures exist.
  - Coordinate with downstream UI consumers (overview tokens, tier summaries) to adopt the shared Resource V2 selectors and retire duplicate percent/badge formatting.
- **Blocking Issues / Risks:** Vitest still fails to resolve `@kingdom-builder/engine` and `@kingdom-builder/testing`, leaving the comprehensive web suite in a failing state until the packages are published or aliased in the sandbox.
