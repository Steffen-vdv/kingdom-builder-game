...
describe('Resource state', () => {
	...
	it('prevents parent lower bounds from exceeding child aggregates', () => {
		...
		expect(() =>
			...
			'Resource parent "resource-parent" cannot raise lower bound beyond its aggregated child total of 0.',
		);
	});
	...
	it('rejects updates for unknown resources', () => {
		...
		).toThrowError('Resource state does not recognise resource "unknown".');
	});
});
