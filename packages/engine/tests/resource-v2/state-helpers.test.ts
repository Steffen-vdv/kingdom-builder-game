...
describe('ResourceV2 state helpers', () => {
	let player: PlayerState;
	const groupDefinition = resourceV2GroupDefinition({
		id: 'group-alpha',
		parent: {
			lowerBound: 2,
			upperBound: 15,
	...
	it('throws when integer assertions receive fractional input', () => {
		expect(() => assertInteger(3.2, '"test" value')).toThrowError(
			'ResourceV2 state expected "test" value to be an integer but received 3.2.',
		);
	});
