...
describe('snapshotAdvance', () => {
	it('clones advance results with skip metadata and complex effects', () => {
		...
		const resourceGain = resourceAmountParams({
			key: resourceKey,
			amount: 3,
		});
		const nestedStatGain = statAmountParams({
			key: statKey,
			amount: 1,
		});
		const chainedGainParams = resourceAmountParams({
			key: resourceKey,
			amount: 1,
		});
		...
		expect(original.effects[0]?.params).toEqual(resourceGain);
	...
	it('omits skip data when advance results do not include skip info', () => {
		...
		const activePlayer = context.activePlayer;
		const resourceKey = getFirstKey(activePlayer.resources);
		const singleGain = resourceAmountParams({
			key: resourceKey,
			amount: 2,
		});
