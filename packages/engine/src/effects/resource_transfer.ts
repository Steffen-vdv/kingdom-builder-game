...
export const resourceTransfer: EffectHandler<TransferParams> = (
	...
	const defenderNext = available - amount;
	defender.resources[key] = defenderNext;
	...
	const attackerCurrent = attacker.resources[key] || 0;
	const attackerNext = attackerCurrent + amount;
	attacker.resources[key] = attackerNext;
	ctx.services.happinessThresholds.handleResourceChange(
		ctx,
		attacker,
		key,
