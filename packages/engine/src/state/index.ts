...
export class PlayerState {
	...
	setResourceCatalog(catalog: RuntimeResourceCatalog | undefined): void {
		...
		this.resetLegacyKeyCaches();
	}

	private resetLegacyKeyCaches(): void {
		clearStringMap(this.legacyKeyIdCaches.resource);
		clearStringMap(this.legacyKeyIdCaches.stat);
		clearStringMap(this.legacyKeyIdCaches.population);
	}
	...
	private resolveLegacyKeyId(kind: LegacyKeyKind, key: string): string {
		const map = this.legacyKeyIdCaches[kind];
