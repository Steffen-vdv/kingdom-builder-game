...
export function setResourceValue(
	...
	if (!lookup || lookup.kind !== 'resource') {
		throw new Error(
			`Resource state does not recognise resource "${resourceId}".`,
		);
	}
	return applyValue(
		context,
...
function adjustResourceBound(
	...
	if (delta <= 0) {
		throw new Error(
			`Resource state expected ${directionLabel}-bound delta for "${resourceId}" to be positive but received ${delta}.`,
		);
	}
	const lookup = resolveResourceDefinition(catalog, resourceId);
	if (!lookup) {
	...
	if (lookup.kind === 'parent') {
		...
		if (direction === 'lower') {
			if (aggregate < nextBound) {
				throw new Error(
					`Resource parent "${resourceId}" cannot raise lower bound beyond its aggregated child total of ${aggregate}.`,
				);
			}
