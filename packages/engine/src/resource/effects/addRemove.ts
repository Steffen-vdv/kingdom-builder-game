...
interface BaseResourceEffectParams extends Record<string, unknown> {
	/**
	 * Identifier of the Resource entry this effect mutates.
	 */
...
function normaliseChange(
	...
	if (change.type === 'amount') {
		...
		if (!Number.isFinite(amount)) {
			throw new Error(
				`Resource effect expected numeric amount change but received ${amount}.`,
			);
		}
		return {
			type: 'amount',
	...
	if (change.type === 'percentFromResource') {
		if (
			...
			throw new Error(
				'Resource percentFromResource change requires a non-empty ' +
					'sourceResourceId.',
			);
		}
		return {
	...
	const modifiers = change.modifiers.map((modifier) => {
		if (!Number.isFinite(modifier)) {
			throw new Error(
				`Resource effect expected numeric percent modifier but received ${modifier}.`,
			);
		}
		return modifier;
	});
...
function applyResourceEffect(
	...
	if (!params) {
		throw new Error('Resource effect is missing required params.');
	}
	const {
		resourceId,
		change: rawChange,
	...
	if (typeof resourceId !== 'string' || !resourceId.length) {
		throw new Error(
			`Resource effect expected a non-empty resourceId. Received: ${JSON.stringify(params)}`,
		);
	}
	...
	if (!definitionLookup) {
		throw new Error(
			`Resource effect referenced unknown resource "${resourceId}".`,
		);
	}
	if (definitionLookup.kind !== 'resource') {
		throw new Error(
			`Resource effect cannot mutate group parent "${resourceId}"; parent values are derived from children.`,
		);
	}
...
/**
 * Handler for `resource:add` Resource effects.
 ...
 */
export const resourceAdd: ResourceEffectHandler = (
	effect,
	context,
	multiplier = 1,
...
/**
 * Handler for `resource:remove` Resource effects. Mirrors `resourceAdd`
 ...
 */
export const resourceRemove: ResourceEffectHandler = (
	effect,
	context,
	multiplier = 1,
