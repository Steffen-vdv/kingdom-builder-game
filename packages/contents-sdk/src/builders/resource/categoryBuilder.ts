...
class ResourceCategoryBuilderImpl implements ResourceCategoryBuilder {
	...
	label(label: string) {
		if (this.labelSet) {
			throw new Error(
				`${builderName} already has label() set. Remove the duplicate call.`,
			);
		}
		if (!label) {
	...
	icon(icon: string) {
		if (this.iconSet) {
			throw new Error(
				`${builderName} already has icon() set. Remove the duplicate call.`,
			);
		}
	...
	description(description: string) {
		if (this.descriptionSet) {
			throw new Error(
				`${builderName} already has description() set. Remove the duplicate call.`,
			);
		}
	...
	primary() {
		if (this.primarySet) {
			throw new Error(
				`${builderName} already has primary() set. Remove the duplicate call.`,
			);
		}
	...
	build(): ResourceCategoryDefinition {
		...
		if (this.contents.length === 0) {
			throw new Error(
				`${builderName} has no contents. ` +
					`Call resource() or group() at least once before build().`,
			);
		}
		return {
			...this.definition,
