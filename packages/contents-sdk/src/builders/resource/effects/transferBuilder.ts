...
export interface ResourceTransferEndpointBuilder {
	...
	/** Flat method: set change to a specific amount (no callback needed) */
	changeAmount(amount: number): this;
	/** Flat method: set change to a percentage (no callback needed) */
...
function ensureFinite(value: number, field: string): void {
	if (!Number.isFinite(value)) {
		throw new Error(
			`${ENDPOINT_BUILDER_NAME} expected ${field} to be a finite number but received ${value}.`,
		);
	}
}
...
function normaliseChange(
	...
	if (change.modifiers.length === 0) {
		throw new Error(
			`${ENDPOINT_BUILDER_NAME} percent change requires at least one modifier.`,
		);
	}
...
function assertReconciliationMode(
	mode: ResourceReconciliationMode | undefined,
	context: string,
): void {
	if (mode && !VALID_RECONCILIATION_MODES.has(mode)) {
		throw new Error(
			`${context} reconciliation mode "${mode}" is invalid. ` +
				`Valid modes: ${[...VALID_RECONCILIATION_MODES].join(', ')}.`,
		);
	}
}
...
function normaliseOptions(
...
): ResourceValueWriteOptions | undefined {
	if (!options) {
		return undefined;
	}
	...
	if (options.suppressRecentEntry !== undefined) {
		if (typeof options.suppressRecentEntry !== 'boolean') {
			throw new Error(
				`${TRANSFER_BUILDER_NAME} expected options.suppressRecentEntry to be boolean when provided.`,
			);
		}
	...
	if (options.skipTierUpdate !== undefined) {
		if (typeof options.skipTierUpdate !== 'boolean') {
			throw new Error(
				`${TRANSFER_BUILDER_NAME} expected options.skipTierUpdate to be boolean when provided.`,
			);
		}
	...
	return Object.keys(result).length > 0
		? (result as ResourceValueWriteOptions)
		: undefined;
}

class EndpointBuilderImpl implements ResourceTransferEndpointBuilder {
	...
	change(configurator: ChangeConfigurator): this {
		if (typeof configurator === 'function') {
			...
			if (built.resourceId !== this.resourceId) {
				throw new Error(
					`${ENDPOINT_BUILDER_NAME} change() cannot override the resourceId. Expected "${this.resourceId}" but received "${built.resourceId}".`,
				);
			}
			if (built.suppressHooks) {
				throw new Error(
					`${ENDPOINT_BUILDER_NAME} does not support suppressHooks(). Remove the suppressHooks() call when configuring donor/recipient changes.`,
				);
			}
	...
	// Flat methods - no callback required
	changeAmount(amount: number): this {
		ensureFinite(amount, 'amount');
		this.changeParams = { type: 'amount', amount };
		return this;
	}
	...
	reconciliation(
		mode: ResourceReconciliationMode = ReconciliationMode.CLAMP,
	): this {
	...
	build(): ResourceTransferEndpointPayload {
		if (!this.changeParams) {
			throw new Error(
				`${ENDPOINT_BUILDER_NAME} requires change() before build().`,
			);
		}
		...
		const normalisedOptions =
			Object.keys(options).length > 0
				? (options as ResourceValueWriteOptions)
				: undefined;
		...
		return {
			...
			...(this.reconciliationMode
				? { reconciliationMode: this.reconciliationMode }
				: {}),
...
function cloneEndpointPayload(
	...
	return {
		...
		...(payload.reconciliationMode
			? { reconciliationMode: payload.reconciliationMode }
			: {}),
		...(normalisedOptions ? { options: normalisedOptions } : {}),
	};
}
...
class ResourceTransferBuilderImpl implements ResourceTransferBuilder {
	...
	build(): ResourceTransferEffectParams {
		if (!this.donorPayload) {
			throw new Error(
				`${TRANSFER_BUILDER_NAME} requires donor() before build().`,
			);
		}
		if (!this.recipientPayload) {
			throw new Error(
				`${TRANSFER_BUILDER_NAME} requires recipient() before build().`,
			);
		}
...
export function transferEndpoint(
	...
	return new EndpointBuilderImpl(resourceId);
}
