...
class UpperBoundBuilderImpl implements ResourceUpperBoundIncreaseBuilder {
	...
	delta(amount: number): this {
		if (!Number.isFinite(amount)) {
			throw new Error(
				`${BUILDER_NAME} expected delta() to receive a finite number but received ${amount}.`,
			);
		}
		if (!Number.isInteger(amount)) {
			throw new Error(
				`${BUILDER_NAME} expected delta() to receive an integer but received ${amount}.`,
			);
		}
		if (amount <= 0) {
			throw new Error(
				`${BUILDER_NAME} expected delta() to be greater than 0 but received ${amount}.`,
			);
		}
...
export function increaseUpperBound(
	...
	return new UpperBoundBuilderImpl(resourceId);
}
