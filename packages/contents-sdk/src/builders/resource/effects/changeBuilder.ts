...
function assertFiniteNumber(value: number, field: string) {
	if (!Number.isFinite(value)) {
		throw new Error(
			`${BUILDER_NAME} expected ${field} to be a finite number but received ${value}.`,
		);
	}
}
...
class ResourceChangeBuilderImpl implements ResourceChangeBuilder {
	...
	private ensureChangeKind(target: ChangeKind) {
		if (this.changeKind && this.changeKind !== target) {
			throw new Error(
				`${BUILDER_NAME} cannot mix amount() and percent(). Remove the existing ${this.changeKind}() call before switching.`,
			);
		}
	...
	percent(modifier: number, ...additional: readonly number[]) {
		...
		if (modifiers.length === 0) {
			throw new Error(
				`${BUILDER_NAME} percent() requires at least one modifier.`,
			);
		}
	...
	roundingMode(mode: ResourceChangeRoundingMode) {
		if (!this.changeKind) {
			throw new Error(
				`${BUILDER_NAME} roundingMode() requires amount() or percent() ` +
					`to be configured first.`,
			);
		}
		if (!VALID_ROUNDING_MODES.has(mode)) {
			throw new Error(
				`${BUILDER_NAME} rounding mode "${mode}" is invalid. ` +
					`Valid modes: ${[...VALID_ROUNDING_MODES].join(', ')}.`,
			);
		}
		this.rounding = mode;
		return this;
	...
	reconciliation(mode: ResourceReconciliationMode = ReconciliationMode.CLAMP) {
		if (!VALID_RECONCILIATION_MODES.has(mode)) {
			throw new Error(
				`${BUILDER_NAME} reconciliation mode "${mode}" is invalid. ` +
	...
	build(): ResourceChangeEffectParams {
		if (!this.changeKind) {
			throw new Error(
				`${BUILDER_NAME} requires amount() or percent() before build().`,
			);
		}
		...
		if (this.changeKind === 'amount') {
			if (this.amountValue === undefined) {
				throw new Error(
					`${BUILDER_NAME} amount() must be called with a numeric value.`,
				);
			}
			return {
