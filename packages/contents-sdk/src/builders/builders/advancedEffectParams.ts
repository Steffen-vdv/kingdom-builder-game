...
export const evaluationTarget = (type: string) =>
...
export function developmentTarget(id?: string) {
	const builder = new EvaluationTargetBuilder(
		EvaluationTargetTypes.Development,
	);
	if (id) {
		builder.id(id);
	}
...
export const populationTarget = () =>
	new EvaluationTargetBuilder(EvaluationTargetTypes.Population);
...
export class AttackParamsBuilder extends ParamsBuilder<{
	...
	override build() {
		if (!this.wasSet('target')) {
			throw new Error(
				'Attack effect is missing a target. Call targetResource(...) or targetBuilding(...) once.',
			);
		}
		return super.build();
	}
