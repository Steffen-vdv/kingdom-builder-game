...
export class PlayerStartBuilder extends ParamsBuilder<PlayerStartConfig> {
	...
	values(values: Record<string, number>) {
		if (!values) {
			throw new Error(
				'Player start values() needs a record. Use {} when nothing changes.',
			);
		}
		...
		return this.set(
			'values',
			{ ...values },
			'Player start already set values(). Remove the extra values() call.',
		);
	}
	...
	resourceBounds(bounds: PlayerStartResourceBoundsInput) {
		if (!bounds) {
			throw new Error(
				'Player start resourceBounds() needs configuration. Use {} when bounds stay defaulted.',
			);
		}
		if (bounds.lower) {
			this.set(
				'resourceLowerBounds',
				{ ...bounds.lower },
				'Player start already set resourceBounds(). Remove the extra resourceBounds() call.',
			);
		}
		if (bounds.upper) {
			this.set(
	...
	override build(): PlayerStartConfig {
		if (this.requireComplete) {
			...
			if (!this.params.values) {
				throw new Error(
					'Player start is missing values(). Call values(...) before build().',
				);
			}
			if (!this.params.resourceLowerBounds) {
				throw new Error(
					'Player start is missing resourceBounds() lower map. Call resourceBounds(...) before build().',
				);
			}
			if (!this.params.resourceUpperBounds) {
				throw new Error(
					'Player start is missing resourceBounds() upper map. Call resourceBounds(...) before build().',
				);
			}
		}
		if (
			!this.wasSet('values') &&
			this.mirroredValuesFromResources &&
