...
import { effect } from '../builders/evaluators/effectBuilder';
...
type ResourcePercentFromResourceParams = ResourceChangeEffectParams &
	Record<string, unknown>;
...
function configureBuilder(
	builder: ResourceChangeBuilder,
	configure?: ChangeBuilderConfigurator,
) {
	if (configure) {
		configure(builder);
	}
...
function applyOptions(
	builder: ResourceChangeBuilder,
	options?: ResourceAmountChangeOptions,
) {
	...
	if (options?.reconciliation) {
		builder.reconciliation(
			typeof options.reconciliation === 'boolean'
...
export function resourceAmountChange(
...
export function resourceAmountChange(
	resource: ResourceKey,
	amount: number,
	configureOrOptions?: ChangeBuilderConfigurator | ResourceAmountChangeOptions,
): ResourceAmountParams {
...
export function resourceTransferAmount(
	resource: ResourceKey,
	amount: number,
	reconciliation?: TransferReconciliationOptions,
...
export function resourceTransferPercent(
	resource: ResourceKey,
	percent: number,
	reconciliation?: TransferReconciliationOptions,
): ResourceTransferParams {
...
/**
 * Creates a resource add effect configuration.
 * Convenience function for building simple resource addition effects.
 */
export function resourceAddEffect(
	resourceId: string,
	amount: number,
): EffectConfig {
