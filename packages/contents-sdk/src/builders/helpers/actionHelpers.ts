 ...
 */
export function createResourceChangeList(
	entries: Iterable<[ResourceKey, number]>,
): ResourceChangeEffectParams[] | undefined {
 ...
 */
export function mapToResourceChangeList(
	map: Partial<Record<ResourceKey, number>> | undefined,
): ResourceChangeEffectParams[] | undefined {
	if (!map) {
		return undefined;
	}
	return createResourceChangeList(
		Object.entries(map) as [ResourceKey, number][],
	);
}
 ...
 */
export function costsToResourceChangeList(
	costs: ReadonlyArray<{ resource: ResourceKey; amount: number }>,
): ResourceChangeEffectParams[] | undefined {
	return createResourceChangeList(
		costs.map(
			({ resource, amount }) => [resource, amount] as [ResourceKey, number],
		),
	);
}
