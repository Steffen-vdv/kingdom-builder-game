...
export interface SessionPlayerStateSnapshot {
	 ...
	 * Always populated now that ResourceV2 snapshots have replaced the
	 * migration era fallbacks.
	 */
	valuesV2: Record<string, number>;
	/**
	 * ResourceV2 lower/upper bound map aligned with {@link valuesV2}. Always
	 * included so clients can clamp projections without checking legacy
	 * resource sets.
	 */
...
export interface SessionGameSnapshot {
	...
	/**
	 * Session-level ResourceV2 catalog. Always emitted so clients can rely
	 * on catalog access without probing for optional data.
	 */
