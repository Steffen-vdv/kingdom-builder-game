...
export interface SessionPlayerStateSnapshot {
	 ...
	 * Always present so clients can treat ResourceV2 totals as the
	 * authoritative source of resource data.
	 */
	valuesV2: Record<string, number>;
	/**
	 * ResourceV2 lower/upper bound map aligned with {@link valuesV2}. The
	 * bounds surface is always included to simplify clamping logic across
...
export interface SessionGameSnapshot {
	...
	/**
	 * Session-level ResourceV2 catalog. Always present so client layers can
	 * rely on a consistent definition source for resource metadata.
	 */
