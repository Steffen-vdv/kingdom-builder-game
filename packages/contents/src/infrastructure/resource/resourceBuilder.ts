...
import type { ResourceBoundType, ResourceBoundValue, ResourceDefinition, ResourceSection, ResourceTierTrack } from './types';
...
class ResourceBuilderImpl implements ResourceBuilder {
	...
	upkeep(resourceId: string, amount: number) {
		if (this.upkeepSet) {
			throw new Error(`${builderName} already configured upkeep(). Remove the duplicate call.`);
		}
		if (!resourceId) {
			throw new Error(`${builderName} upkeep() requires a non-empty resourceId.`);
		}
	...
	onPayUpkeepStep(...effects: EffectDef[]) {
		if (this.onPayUpkeepStepSet) {
			throw new Error(`${builderName} already configured onPayUpkeepStep(). ` + 'Remove the duplicate call.');
		}
	...
	onGainIncomeStep(...effects: EffectDef[]) {
		if (this.onGainIncomeStepSet) {
			throw new Error(`${builderName} already configured onGainIncomeStep(). ` + 'Remove the duplicate call.');
		}
	...
	onGainAPStep(...effects: EffectDef[]) {
		if (this.onGainAPStepSet) {
			throw new Error(`${builderName} already configured onGainAPStep(). ` + 'Remove the duplicate call.');
		}
