/**
 * Happiness Resource Builder (Infrastructure)
 *
 * This file contains technical code for building the happiness resource
 * definition with its tier system. The lazy initialization breaks circular
 * dependencies with effect builders.
 *
 * The actual happiness resource definition is exposed via
 * getHappinessResourceDefinition() and consumed by resources.ts.
 */
import { PassiveMethods, Types, passiveParams, resource } from '@boardsmith/contents-sdk';
import type { ResourceTierDefinition, ResourceDefinition } from '@boardsmith/contents-sdk';
...
let cachedHappinessTiers: readonly ResourceTierDefinition[] | null = null;

function getHappinessTiers(): readonly ResourceTierDefinition[] {
	...
	cachedHappinessTiers = getTierConfigs().map((config, index) => {
		...
		const tier: ResourceTierDefinition = {
			id: config.id,
			label,
			icon,
...
let cachedHappinessDefinition: ResourceDefinition | null = null;

export function getHappinessResourceDefinition(): ResourceDefinition {
	if (cachedHappinessDefinition) {
		return cachedHappinessDefinition;
	}

	cachedHappinessDefinition = resource(HAPPINESS_RESOURCE_ID)
		.icon('😊')
		.label('Happiness')
