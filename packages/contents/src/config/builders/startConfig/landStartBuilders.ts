import type { EffectConfig, PlayerStartConfig } from '@kingdom-builder/protocol';
...
export function cloneLandStartConfig(land: LandStartConfig): LandStartConfig {
	return {
		...
		onPayUpkeepStep: land.onPayUpkeepStep ? [...land.onPayUpkeepStep] : undefined,
		onGainIncomeStep: land.onGainIncomeStep ? [...land.onGainIncomeStep] : undefined,
...
export class PlayerStartLandBuilder extends ParamsBuilder<LandStartConfig> {
	...
	slotsMax(slots: number) {
		return this.set('slotsMax', slots, 'Player start land already set slotsMax(). Remove the extra slotsMax() ' + 'call.');
	}

	slotsUsed(slots: number) {
		return this.set('slotsUsed', slots, 'Player start land already set slotsUsed(). Remove the extra slotsUsed() ' + 'call.');
	}

	tilled(tilled = true) {
		return this.set('tilled', tilled, 'Player start land already set tilled(). Remove the extra tilled() call.');
	}

	upkeep(costs: Record<string, number>) {
		return this.set('upkeep', { ...costs }, 'Player start land already set upkeep(). Remove the extra upkeep() call.');
	}

	onPayUpkeepStep(...effects: Array<EffectConfig | EffectBuilder>) {
		this.params.onPayUpkeepStep ??= [];
		this.params.onPayUpkeepStep.push(...effects.map((effect) => resolveEffectConfig(effect)));
		return this;
	}

	onGainIncomeStep(...effects: Array<EffectConfig | EffectBuilder>) {
		this.params.onGainIncomeStep ??= [];
		this.params.onGainIncomeStep.push(...effects.map((effect) => resolveEffectConfig(effect)));
		return this;
	}

	onGainAPStep(...effects: Array<EffectConfig | EffectBuilder>) {
		this.params.onGainAPStep ??= [];
		this.params.onGainAPStep.push(...effects.map((effect) => resolveEffectConfig(effect)));
		return this;
	}
...
type PlayerStartLandBuilderCallback = (builder: PlayerStartLandBuilder) => PlayerStartLandBuilder;

type PlayerStartLandInput = LandStartConfig | PlayerStartLandBuilder | PlayerStartLandBuilderCallback;

export class PlayerStartLandsBuilder {
	private readonly lands: LandStartConfig[] = [];

	private resolveBuilder(input: PlayerStartLandBuilder | PlayerStartLandBuilderCallback) {
		...
		if (!(configured instanceof PlayerStartLandBuilder)) {
			throw new Error('Player start lands land(...) callback must return the provided builder.');
		}
		return configured;
	}
