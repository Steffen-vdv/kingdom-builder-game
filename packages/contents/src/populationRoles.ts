import { populationRole, type PopulationRoleInfo, toRecord } from './config/builders';
...
const defs: PopulationRoleInfo[] = [
	populationRole(PopulationRole.Council)
		...
		.build(),
	populationRole(PopulationRole.Legion).icon('🎖️').label('Legion').description('Legions lead your forces, boosting Army Strength and training troops each Growth phase.').build(),
...
export const POPULATION_ROLES: Record<PopulationRoleId, PopulationRoleInfo> = toRecord(defs) as Record<PopulationRoleId, PopulationRoleInfo>;
