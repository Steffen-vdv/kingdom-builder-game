...
const devSlotReq: RequirementConfig = compareRequirement().left(landEvaluator()).operator('gt').right(0).message('Requires an available development slot.').build();
...
function buildActions(): ActionEntry[] {
	return BUILDINGS.map(({ actId, bldId, name, icon, order }) => {
		const def = action()
			...
			.icon(icon)
			.locked()
			.effect(effect(Types.Building, BuildingMethods.ADD).params(buildingParams().id(bldId).build()).build())
...
const DEVELOPMENTS: ReadonlyArray<{
	...
	{
		actId: Act.developTradingPost,
		devId: Dev.tradingPost,
		name: 'Trading Post',
		icon: '🏪',
		order: 205,
		costs: [
...
function developActions(): ActionEntry[] {
	return DEVELOPMENTS.map(({ actId, devId, name, icon, order, costs, locked }) => {
		const builder = action().id(actId).metaCategory(MetaCat.commands).name(name).icon(icon);
		...
		builder
			...
			.effect(effect(Types.Development, DevelopmentMethods.ADD).params(developmentParams().id(devId).landId('$landId').build()).build())
