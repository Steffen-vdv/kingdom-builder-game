...
function createActionsPanelScenario() {
	...
	Object.assign(mockGame.requests, {
		hasAiController: vi.fn((playerId: string) => playerId === aiPlayer.id),
		readActionMetadata: vi.fn(() => metadataSnapshot),
		subscribeActionMetadata: vi.fn(() => () => {}),
		getActionCosts: vi.fn(() => metadataSnapshot.costs ?? {}),
		getActionRequirements: vi.fn(() => metadataSnapshot.requirements ?? []),
	});
	return {
		mockGame,
		actionCostResource,
