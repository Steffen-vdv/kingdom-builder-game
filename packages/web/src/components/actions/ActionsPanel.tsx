...
function RaisePopOptions({
	...
	const multiStepIndicator: MultiStepConfig | undefined = useMemo(() => {
		if (!actionDefinition?.effectGroups?.length) return undefined;
		const [firstGroup] = actionDefinition.effectGroups;
		return {
			icon: '🔀',
			isActive: false,
			totalSteps: actionDefinition.effectGroups.length,
			currentStep: 0,
			...(firstGroup?.title ? { title: firstGroup.title } : {}),
			contentKey: 'front',
		} satisfies MultiStepConfig;
	}, [actionDefinition]);
