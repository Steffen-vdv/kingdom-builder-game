...
import { resolvePlayerAccentClass } from './common/playerAccent';
import {
	buildTimelineTree,
...
function ResolutionCard({
	...
	const optionalGame = useOptionalGameEngine();
	const players = optionalGame?.sessionSnapshot.game.players ?? [];
	const playerLabel = resolution.player?.name ?? resolution.player?.id ?? null;
	const playerName = playerLabel ?? 'Unknown player';
	const accentClass = resolvePlayerAccentClass(
		resolution.player?.id ?? null,
		players,
	);
