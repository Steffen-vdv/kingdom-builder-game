...
const HERO_PARAGRAPH_TEXT = [
	...
	'{build} clever construction, and {army_attack} daring raids decide who',
	'steers the crown.',
].join(' ');
...
const DEFAULT_SECTIONS: OverviewSectionTemplate[] = [
		...
		items: [
			{
				...
				body: [
					...
					'Stacks {fortificationStrength} Fortification bonuses and triggers',
					'automatic boons.',
				],
			},
			{
	...
	},
	{
		...
		paragraphs: [
			...
			'Farms pump {gold} gold while signature projects open slots or unleash',
			'passives.',
		],
	},
	{
		...
		items: [
			...
			},
			{
				...
				label: 'Legion',
				body: [
					'Reinforces {armyStrength} Army strength for devastating',
					'{army_attack} raids.',
				],
			},
			{
	...
	},
	{
		...
		span: true,
		paragraphs: [
			'Spend {ap} AP to {expand} grow territory or {develop} upgrade key',
			'lands.',
			'Field {raise_pop} specialists or launch {army_attack} attacks to',
			'snowball momentum.',
		],
	},
];
...
export const DEFAULT_OVERVIEW_SECTIONS = DEFAULT_SECTIONS;
export const DEFAULT_OVERVIEW_TOKENS = DEFAULT_TOKENS;
