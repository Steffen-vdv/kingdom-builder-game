...
import type { PlayerSnapshot, TranslationDiffContext } from '../translation';
...
/**
 * Builds the formatted log artefacts for an action resolution. Consumers
 * provide the translation context, player snapshots, optional parameters, and
 * any trace metadata returned by the engine. The helper returns the timeline
 * entries, formatted log lines, and diff summaries used by action resolution
 * dialogs.
 */
...
function filterActionDiffChanges({
	changes,
	messages,
	subLines,
...
export function buildActionResolution({
	...
	const rawMessages = logContent('action', actionId, context, params);
