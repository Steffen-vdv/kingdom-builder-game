...
export function GameProviderInner({
	...
	useEffect(() => {
		...
		void enqueue(async () => {
			const response = await updateSessionPlayerName({
				sessionId,
				playerId: primaryPlayerId,
				playerName: desiredName,
			});
			const updatedPlayer = response.snapshot.game.players.find(
				(entry) => entry.id === primaryPlayerId,
			);
			const sanitizedName = updatedPlayer?.name ?? desiredName;
			legacySession.updatePlayerName(primaryPlayerId, sanitizedName);
			return response;
		}).finally(() => {
			refresh();
		});
