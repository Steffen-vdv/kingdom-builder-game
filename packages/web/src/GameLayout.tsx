...
export default function GameLayout() {
	...
	const localPlayerId = sessionState.game.players[0]?.id;
	const outcomePlayers = sessionState.game.players.map(({ id, name }) => ({
		id,
		name,
	}));
	const showOutcome = outcome.status === 'finished';
	const phasePanelElement = (
		<div className="w-full lg:col-start-2">
			<PhasePanel height={phasePanelHeight} />
		</div>
	);
	const outcomeProps = localPlayerId ? { localPlayerId } : {};
	const logButton = (
		<Button
			onClick={toggleLog}
			variant="secondary"
