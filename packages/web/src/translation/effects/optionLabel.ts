...
function resolveActionLabel(
	...
	try {
		...
		const icon =
			typeof definition?.icon === 'string' ? definition.icon.trim() : undefined;
		const name =
		...
		const headline = formatActionHeadline(headlineOptions);
		return headline.length > 0 ? headline : option.actionId;
	} catch {
