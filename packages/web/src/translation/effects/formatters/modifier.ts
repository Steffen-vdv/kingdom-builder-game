...
const TRANSFER_ICON_FALLBACK = '🔁';

function selectTransferIcon(context: TranslationContext): string {
	const descriptor = context.assets.assets?.['resource:transfer'];
	if (descriptor?.icon && descriptor.icon.trim().length > 0) {
		return descriptor.icon;
	}
	return TRANSFER_ICON_FALLBACK;
}
...
registerModifierEvalHandler('transfer_pct', {
	summarize: (effect, evaluation, context) => {
		...
		const transferIcon = selectTransferIcon(context);
