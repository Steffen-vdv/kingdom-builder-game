...
export function formatResourceDiff(
        ...
        const descriptor = selectResourceIconLabel(context, diff.key as string);
...
context: TranslationContext,
        ...
        const descriptor = selectStatIconLabel(context, diff.key as string, {
                fallbackLabel: diff.key as string,
        });
        const displayLabel = iconLabel(descriptor.icon, descriptor.label);
        const delta = diff.after - diff.before;
