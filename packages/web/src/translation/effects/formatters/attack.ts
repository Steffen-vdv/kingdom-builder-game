...
function fallbackLog(
	...
	const baseEntry = buildBaseEntry(effectDefinition, translationContext, 'describe');
...
export function buildOnDamageEntry(
	...
	const items: SummaryEntry[] = [];
	const defenderEntries = logEntries.filter((entry) => entry.owner === 'defender');
	const attackerEntries = logEntries.filter((entry) => entry.owner === 'attacker');
