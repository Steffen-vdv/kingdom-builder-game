...
import type { TranslationInfo } from '../context/defaultInfo';
...
function decoratePassiveLabel(
        ...
        const fallback = label.trim() || info.passive.label || label;
        const decorated = [icon, fallback]
                .filter((part) => part && String(part).trim().length > 0)
                .join(' ')
                .trim();
        const prefix = info.passive.icon?.trim();
        if (!prefix) {
                return decorated;
        }
	...
	for (const [id, passive] of next) {
		if (previous.has(id)) {
			continue;
                ...
                const { icon, label } = resolvePassivePresentation(passive, { info });
                const decoratedLabel = decoratePassiveLabel(icon, label, info);
