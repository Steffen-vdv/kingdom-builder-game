...
- 2025-12-06: PlayerState uses `resourceTouched` to track resources whose values
  have ever been non-zero. Non-primary category resources only display in the UI
  when touched. Primary categories (marked with `isPrimary: true`) always show
  all their resources.
- 2025-08-31: Restore a file from the previous commit with
  `git checkout HEAD^ -- path/to/file`.
- 2025-09-01: Resource effects honor a `round` setting; use `round: 'up'` to
  keep strength values integer and non-negative.
...
- **ResourceV2 (Unified Resource System)**
  - Definition: All numeric values—currencies, stats, population roles—are
    unified under the ResourceV2 system. Values are stored in `resourceValues`
    with `resourceTouched` tracking whether values have ever been non-zero.
    Resources are organized into groups and categories for UI display.
  - Categories: Resources are grouped into categories marked as primary (always
    visible) or non-primary (visible only when touched). Use `isPrimary: true`
    on category definitions to mark core resources.
  - Effects: Modified via `resource:add`, `resource:remove`, or
    `resource:transfer` effects with `resourceId` parameters. The old separate
    `stat:*` and `population:*` effect types are deprecated.
  - Use: Currencies (gold, AP), stats (army strength, absorption), and
    population counts (council, legion) are all resources with different
    semantic meanings but unified mechanics.
  - Contrast: Groups aggregate child resources (e.g., total population);
    categories organize display in the UI.
- **Buildings**
...
Population roles are now unified under ResourceV2 as resources within a
population group. Each role is a resource with triggers and effects.
...
- Removing a role reduces its resource value; associated effects update
  accordingly.
