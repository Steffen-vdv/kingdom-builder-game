...
| **Population**                | Counts of individuals per role in a `population` map. Role IDs are configurable and each role can trigger `onAssigned`/`onUnassigned`/phase effects. Added or removed via `population:add`/`population:remove` which also fire role effects and passives             | Models workers/units such as Council or Commander. Used for income, strength bonuses, or other triggered effects.             | Unlike **Stats**, population is discrete and can’t be modified fractionally. It is capped by rules (pop cap) rather than history.                             |
